        ...
        if not predo_data.empty:
            ...
            if len(cases) == 1:
                ...
                query = f"""
                        UPDATE `queue_list`
                        SET 
                            `case_creation_status` = IF(count_of_tries >= 3, 'Failed', 'Picked')
                        WHERE `case_id` = '{case}'
                    """
        ...
        for index, row in chunk.iterrows():
            ...
            if response.status_code == 500:
                ...
                query_error = f"""
                                UPDATE `queue_list`
                                SET 
                                    `case_creation_status` = IF(count_of_tries = 3, 'Failed', NULL),
                                    `count_of_tries` = `count_of_tries` + 1,
                                    `error_logs` = '{message}'
...
def uam_dormancy_scheduler(tenant_id):
    ...
    try:
        dormant_rules_query = f"SELECT first_login_day, login_day from dormant_rules"
        ...
        logging.info(f"first_login_day_limit :{first_login_day_limit},{login_day_limit}")
        ...
        logging.info(f"act_dir :{act_dir}")
        ...
        logging.info(f"live_sess : {live_sess}")
        userss = [entry['USER_'] for entry in live_sess]
        logging.info(f"act_dir :{userss}")
    ...
    try:
        ...
        for user in act_dir:
            ...
            try:
                ...
                elif status == "lock" and created_date == updated_date:
                    dormant_users.append(username)
                    dormant_user_count += 1
...
if __name__ == '__main__':
    while True:
        ...
        auto_dormancy_process = multiprocessing.Process(target=run_schedule, args=(uam_dormancy_scheduler,None,"18:29", 'hdfc'))
